...
config const datasetDirectory = "/lustre/firo017/chgl_upgrade/homology/chgl/homology_data_emilie/";
/*
  Output directory.
*/
...
for (_kCellsArray, kCellKey) in zip(kCellsArrayMap, kCellKeys) {
  ...
  if (kCellKey == 3) {
    writeln("size: " + _kCellsArray.A.size);
    for c in _kCellsArray.A {
      if (c == "") {
	writeln("Found an empty string!");
      }
      writeln(c);
    }
  }
