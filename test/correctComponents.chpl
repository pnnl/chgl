...
// Required or else compiler causes a fit about initializing vPropMap and ePropMap using
// graph before it is initialized (graph requires both vPropMap and ePropMap to be initialized...)
{
    ...
    for s in 1..3 {
        ...
        f.writeln("Vertex Connected Component Size Distribution (s = " + s : string + "):");
        for (sz, freq) in zip(vComponentSizes.domain, vComponentSizes) {
            if freq != 0 then f.writeln("\t" + sz : string + "," + freq : string);
        }
        f.flush();

        f.writeln("Edge Connected Component Size Distribution (s = " + s : string + "):");
