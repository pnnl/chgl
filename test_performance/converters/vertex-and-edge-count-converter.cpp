...
/*
   void mm_fill(std::istream& inputStream, edge_list& A, size_t nNonzeros, bool file_symmetry, bool pattern) {
   ...
   A.push_back(std::make_tuple(d0, d1));

   if (file_symmetry && (d0 != d1)) {
   A.push_back(std::make_tuple(d1, d0));
   }
   }
   }

   template<int idx>
   void stable_sort_by(edge_list& edgeList) {
   std::sort(edgeList.begin(), edgeList.end(),
   [](const element& a, const element& b) -> bool { return (std::get<idx>(a) < std::get<idx>(b)); });
   }

// void fill(graph_adjacency_list& adjacency_list, edge_list& edgeList) {
//   stable_sort_by<0>(edgeList);
//   for (auto& elt : edgeList) {
//     adjacency_list.insert(std::get<0>(elt), std::get<1>(elt));
//   }
//   // csr.accumulate();
// }


void read_mm(std::istream& inputStream, edge_list& edgeList,
ve_type& nVertices, ve_type& nEdges) {
std::string              string_input;
bool                     file_symmetry = false;
std::vector<std::string> header(5);
...
mm_fill(inputStream, edgeList, nNonzeros, file_symmetry, (header[3] == "pattern"));

}

void read_mm(const std::string& filename, edge_list& edgeList,
        ve_type& nVertices, ve_type& nEdges) {
    std::ifstream inputFile(filename);
    read_mm(inputFile, edgeList, nVertices, nEdges);
}
